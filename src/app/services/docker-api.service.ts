  ...
  */
  private createOptions(): RequestOptions {
    const headers: Headers = new Headers({
      'Content-Type': 'application/json' /*,'Authorization':  this.basicAuth()*/
    });
  ...
  private basicAuth(): string {
    return (
      'Basic ' + btoa(this.credential.username + ':' + this.credential.password)
    );
  }
  ...
  public manifests(image: string, reference: string): Observable<Manifest> {
    ...
    return this.http
      ...
      .map((res: Response) => {
        const data: Object = this.extractData(res);
        const infos: Object = data['history']
          ? JSON.parse(data['history'][0]['v1Compatibility'])
          : {};
